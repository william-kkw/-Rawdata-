...
output_path = os.path.join(p,
                            '01. 3CE企微Monthly Report_去重名单.xlsx'
                            )
...
for r in regions:
    ...
    for store in stores_by_region:
        ...
        df5 = df_BA[df_BA['store_name'] == store].copy()
...
# Step7: 修改原始数据中的对应部分的内容
with pd.ExcelWriter(os.path.join(p, '02. 3CE企微Monthly Report_RawData.xlsx'), engine='openpyxl') as writer:
    for k in dict_dfs.keys():
        df_data = pd.DataFrame(dict_dfs[k])
        df_data.to_excel(writer, sheet_name=k, index=False)
print(f"🎉 {Fore.GREEN}Raw Data has successfully been output to {p}")